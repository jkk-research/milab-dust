...
public class DustHttpServerJetty implements DustNetConsts, DustConsts.MindAgent, DustConsts.DustThreadOwner {
	enum Commands {
		stop, info,
	}
...
//					DustException.swallow(e);
	...
	private void init() throws Exception {
		if ( null == jetty ) {
			...
			Long sslPort = Dust.access(MindContext.Self, MindAccess.Peek, null, NET_ATT_SSLINFO_PORT);
			...
			if ( null != sslPort ) {
				...
				String str;
				str = Dust.access(MindContext.Self, MindAccess.Peek, null, NET_ATT_SSLINFO_STOREPATH);
				sslContextFactory.setKeyStorePath(ClassLoader.getSystemResource(str).toExternalForm());
				str = Dust.access(MindContext.Self, MindAccess.Peek, null, NET_ATT_SSLINFO_STOREPASS);
				sslContextFactory.setKeyStorePassword(str);
				str = Dust.access(MindContext.Self, MindAccess.Peek, null, NET_ATT_SSLINFO_KEYMANAGERPASS);
	...
	private void process(Commands cmd) throws Exception {
		switch ( cmd ) {
		case stop:
			new Thread() {
				@Override
				public void run() {
					try {
						Dust.log("Shutting down Jetty server...");
						release();
						Dust.log("Jetty server shutdown OK.");
					} catch (Exception ex) {
